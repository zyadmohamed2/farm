...
import 'package:napta/modules/farm/presentation/bloc/market_products_bloc/create_market_product_bloc.dart';
...
class AddProductToMarket extends StatelessWidget {
  var inAsyncCall = ValueNotifier<bool>(false);
  static final formKey = GlobalKey<FormState>();
  final product =
      ValueNotifier<MarketProductModel>(MarketProductModel(userName: "كجم"));

  AddProductToMarket({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocListener<CreateMarketProductBloc, MarketProductsStatus>(
      listener: (context, state) {
        if (state is LoadingMarketProductsStatus) {
          inAsyncCall.value = true;
        } else if (state is MarketProductCreatedSuccessStatus) {
        ...
        } else if (state is MarketProductCreatedFailedStatus) {
          inAsyncCall.value = false;
      ...
      child: ValueListenableBuilder<bool>(
        ...
        builder: (context, value, child) {
          log(product.value.farmMetaDataId.toString());
                    ...
                    child: Form(
                      ...
                      child: Column(
                        children: [
                          ...
                          BlocConsumer<MarketProductTypesBloc,
                              MarketProductTypesStatus>(
                            listener: (context, state) {
                              if (state is MarketProductTypesSuccessStatus) {
                                product.value.unitName = state
                                    .controller[1]
                                    ?.data[state.controller[2]?.selectedId!]
                                    ?.name;
                              }
                            },
                          ...
                          CustomTextFormFiled(
                              ...
                              suffix: Column(
                                ...
                                children: [
                                  (product.value.farmMetaDataId ?? 0) != 0
                                      ? Padding(
                                          padding: EdgeInsets.symmetric(
                                              horizontal:
                                                  15.responsive(context)),
                                          child: CustomText(
                                            title:
                                                "${product.value.unitName.toString()} / جنيه",
                                            color: ColorManager.black,
                                          ),
                                        )
                                      : Padding(
                            ...
                            suffix: Column(
                              ...
                              children: [
                                Padding(
                                  ...
                                  child: CustomText(
                                    title: product.value.quantity == null
                                        ? ""
                          ...
                          CustomButton(
                            ...
                            onTap: () {
                              if (formKey.currentState?.validate() == true) {
                                ...
                                BlocProvider.of<CreateMarketProductBloc>(
                                        context)
                                    .add(CreateMarketProductEvent(
                                        model: product.value));
                              }
                            },
                          ),
